...
class UserRepository {
  ...
  async createUser(user) {
    ...
    const [result] = await db.query('INSERT INTO users (id, email, nickname, password, active) VALUES (?,?,?,?,?);', [null, user.email, user.nickname, user.password, new Date()]); //, crypto.randomBytes(32).toString('hex')
  ...
  async loginUser(user) {
    ...
    const [result] = await db.query('SELECT id FROM users WHERE email=? AND password=?;', [user.email, user.password]); //, crypto.randomBytes(32).toString('hex')
  ...
  async getAllUsers() {
    console.log(`[${(new Date()).toISOString()}] - ASYNC UserRepository.createUser [IN] - ${ JSON.stringify() }`);
    const [rows] = await db.query('SELECT * FROM users');
    console.log(`[${(new Date()).toISOString()}] - ASYNC UserRepository.createUser [OUT] - ${ JSON.stringify(rows) }`);
    return rows;
  }
}
